...
func (p *masterChallengeProvider) presentDNS01(domain, token, keyAuth string) error {
	...
	utils.CertLogf("acme: dns01 present hostname=%s order_id=%s token=%s fqdn=%s", p.domain.Hostname, p.orderID, token, info.EffectiveFQDN)
	...
	if err := p.service.repo.DNSRecords().UpsertResource(ctx, record); err != nil {
		utils.CertLogf("acme: dns01 persist failed hostname=%s token=%s record_id=%s err=%v", p.domain.Hostname, token, record.ID, err)
		return err
	}
...
func (p *masterChallengeProvider) cleanupDNS01(_ string, token, _ string) error {
	ctx := context.Background()
	utils.CertLogf("acme: dns01 cleanup hostname=%s order_id=%s token=%s record_id=%s", p.domain.Hostname, p.orderID, token, p.dns01RecordID(token))
