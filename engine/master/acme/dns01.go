...
func (p *masterChallengeProvider) presentDNS01(domain, token, keyAuth string) error {
	...
	log.Printf("acme: dns01 present hostname=%s order_id=%s token=%s fqdn=%s", p.domain.Hostname, p.orderID, token, info.EffectiveFQDN)
	...
	if err := p.service.repo.DNSRecords().UpsertResource(ctx, record); err != nil {
		log.Printf("acme: dns01 persist failed hostname=%s token=%s record_id=%s err=%v", p.domain.Hostname, token, record.ID, err)
		return err
	}
	log.Printf("acme: dns01 persisted hostname=%s token=%s record_id=%s", p.domain.Hostname, token, record.ID)
...
func (p *masterChallengeProvider) cleanupDNS01(_ string, token, _ string) error {
	ctx := context.Background()
	log.Printf("acme: dns01 cleanup hostname=%s order_id=%s token=%s record_id=%s", p.domain.Hostname, p.orderID, token, p.dns01RecordID(token))
	if err := p.service.repo.DNSRecords().DeleteResourceByField(ctx, &metadata.DNSRecord{}, "id", p.dns01RecordID(token)); err != nil {
		log.Printf("acme: dns01 cleanup failed hostname=%s token=%s err=%v", p.domain.Hostname, token, err)
		return err
	}
	return p.service.publishChange(ctx, p.domain.Hostname)
}
