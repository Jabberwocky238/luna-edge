...
type grpcSnapshotStream struct { stream replpb.ReplicationService_GetSnapshotClient }
...
type grpcNoticeStream struct { stream replpb.ReplicationService_SubscribeClient }
...
func SnapshotToProto(in *Snapshot) *replpb.Snapshot {
    ...
    for i := range in.DNSRecords { out.DnsRecords = append(out.DnsRecords, dnsRecordToProto(in.DNSRecords[i])) }
    out.DomainEntries = make([]*replpb.DomainEntryProjection, 0, len(in.DomainEntries))
    for i := range in.DomainEntries { out.DomainEntries = append(out.DomainEntries, domainEntryProjectionToProto(in.DomainEntries[i])) }
    return out
}
...
func SnapshotFromProto(in *replpb.Snapshot) *Snapshot {
    ...
    for _, item := range in.GetDnsRecords() { out.DNSRecords = append(out.DNSRecords, dnsRecordFromProto(item)) }
    out.DomainEntries = make([]metadata.DomainEntryProjection, 0, len(in.GetDomainEntries()))
    for _, item := range in.GetDomainEntries() { out.DomainEntries = append(out.DomainEntries, domainEntryProjectionFromProto(item)) }
    return out
}
...
func dnsRecordFromProto(in *replpb.DNSRecord) metadata.DNSRecord {
    if in == nil { return metadata.DNSRecord{} }
...
func httpRouteProjectionFromProto(in *replpb.HTTPRouteProjection) metadata.HTTPRouteProjection {
    if in == nil { return metadata.HTTPRouteProjection{} }
...
func certificateRevisionToProto(in *metadata.CertificateRevision) *replpb.CertificateRevision {
    if in == nil { return nil }
    return &replpb.CertificateRevision{Id: in.ID, DomainEndpointId: in.DomainEndpointID, Hostname: in.Hostname, Revision: in.Revision, Provider: in.Provider, ChallengeType: string(in.ChallengeType), ArtifactBucket: in.ArtifactBucket, ArtifactPrefix: in.ArtifactPrefix, Sha256Crt: in.SHA256Crt, Sha256Key: in.SHA256Key, NotBefore: timeToProto(in.NotBefore), NotAfter: timeToProto(in.NotAfter)}
}

func certificateRevisionFromProto(in *replpb.CertificateRevision) *metadata.CertificateRevision {
    if in == nil { return nil }
    return &metadata.CertificateRevision{ID: in.GetId(), DomainEndpointID: in.GetDomainEndpointId(), Hostname: in.GetHostname(), Revision: in.GetRevision(), Provider: in.GetProvider(), ChallengeType: metadata.ChallengeType(in.GetChallengeType()), ArtifactBucket: in.GetArtifactBucket(), ArtifactPrefix: in.GetArtifactPrefix(), SHA256Crt: in.GetSha256Crt(), SHA256Key: in.GetSha256Key(), NotBefore: timeFromProto(in.GetNotBefore()), NotAfter: timeFromProto(in.GetNotAfter())}
}
...
func domainEntryProjectionToProto(in metadata.DomainEntryProjection) *replpb.DomainEntryProjection {
    ...
    for i := range in.HTTPRoutes { out.HttpRoutes = append(out.HttpRoutes, httpRouteProjectionToProto(in.HTTPRoutes[i])) }
    return out
}

func domainEntryProjectionFromProto(in *replpb.DomainEntryProjection) metadata.DomainEntryProjection {
    if in == nil { return metadata.DomainEntryProjection{} }
    ...
    for _, item := range in.GetHttpRoutes() { out.HTTPRoutes = append(out.HTTPRoutes, httpRouteProjectionFromProto(item)) }
    return out
}
...
func CollectSnapshotStream(stream SnapshotStream) ([]*Snapshot, error) {
    ...
    for {
        ...
        out = append(out, item)
        if item != nil && item.Last { return out, nil }
    }
}
