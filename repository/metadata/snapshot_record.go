...
type SnapshotRecord struct {
	...
	// SyncType 表示同步对象类型。
	SyncType SnapshotSyncType `json:"sync_type" gorm:"column:sync_type;not null;type:varchar(64);index"`
	// SyncID 表示对应业务表中的对象 ID。
	SyncID string `json:"sync_id" gorm:"column:sync_id;not null;type:varchar(64);index"`
	// Action 表示此次同步动作。
	Action SnapshotAction `json:"action" gorm:"column:action;not null;type:varchar(16)"`
