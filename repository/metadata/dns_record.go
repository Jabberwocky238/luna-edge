...
type DNSRecord struct {
	...
	// FQDN 是该记录对应的完整域名。
	FQDN string `json:"fqdn" gorm:"column:fqdn;not null;index:idx_dns_lookup,priority:1;type:text"`
	// RecordType 是该记录类型，例如 A、AAAA、TXT。
	RecordType DNSRecordType `json:"record_type" gorm:"column:record_type;not null;index:idx_dns_lookup,priority:2;type:text"`
	// RoutingClass 表示该记录使用的路由类别，例如 simple 或 geo。
	RoutingClass RoutingClass `json:"routing_class" gorm:"column:routing_class;not null;default:simple;type:text"`
	...
	RoutingKey string `json:"routing_key" gorm:"column:routing_key;not null;default:'';type:text"`
